...
function BuildAbox() {
  ...
  useEffect(() => {
    fetch(`${supabaseUrl}?select=*`, { headers: supabaseHeaders })
      .then((res) => res.json())
      .then((data) => {
        const initial = data.map((p) => ({ ...p, order_count: 0 }));
        setProducts(initial);
      })
  ...
  const handlePackClick = (packNumber) => {
    ...
    if (totalSelected > packNumber) {
      alert(
        `You already selected ${totalSelected} cookies, but the new pack size is ${packNumber}.
Please remove some items to fit the new pack size.`
  ...
  const isPackFull = totalSelected === selectedPack;

  const handleAddToCart = () => {
    const cartString = localStorage.getItem('myCart');
    let cartArray = cartString ? JSON.parse(cartString) : [];
    const newOrder = {
    ...
    products.forEach((p) => {
      if (p.order_count > 0) {
        newOrder.counts[p.id] = p.order_count;
      }
    });

    cartArray.push(newOrder);
    ...
    <section className="min-h-screen w-full text-white relative mt-10">
      <div className="max-w-4xl mx-auto px-4 py-10 text-center">
        ...
        <p className="text-gray-300 max-w-2xl text-2xl mx-auto mb-6">
          Go ahead, stuff your box. Select from a 6, 12, or 18 pack and choose your
          favorite flavors. Prepare to <span className="font-semibold">#GETBLAKED</span>.
        </p>

        <div className="flex flex-wrap justify-center gap-4 mb-4">
          <button
            onClick={() => handlePackClick(6)}
            className={`px-6 py-3 rounded-[7px] font-bold border-1 transition-colors cursor-pointer ${
      ...
      <div
        style={{ background: 'rgb(50,47,48)' }}
        className="fixed bottom-0 left-0 w-full py-4 overflow-x-auto flex flex-col"
      >
        ...
        {isPackFull && (
          <button
            ...
            className="w-[200px] md:w-[300px] ms-auto me-auto mt-3 px-6 py-2 rounded-[5px] cursor-pointer border-2 border-white hover:bg-[#555] font-bold text-center"
          >
            ADD TO CART
          </button>
        )}
